...
HELP = '''Correct usage:
$ login -- Efetuar o login do cliente no sistema.
...
def encrypt_with_certificate(certificado, mensagem):
    # Carrega o certificado
    client_cert_reloaded = x509.load_pem_x509_certificate(certificado)
    
    # Cifra a mensagem usando a chave pública do certificado
    ciphertext = client_cert_reloaded.public_key().encrypt(
        mensagem,
        padding.OAEP(
...
def validaRecibo(recibo, certificado):
    message, sig = unpair(recibo)
    if not valida_assinatura2(certificado, sig, message):
        sys.stderr.write("MSG RELAY SERVICE: verification error!\n")
    print("RECIBO: ", message.decode())
...
class Client:
    ...
    def handle_send_message(self, certificado, count):
        ...
        #Envia mensagem
            ...
            if count == 0:
                data = {"resposta": mkpair(b'sendF1', mkpair(self.args[1].encode(), self.clientID.encode()))}
                ...
                if certificado is not None:
                    ...
                    # Carregar o certificado do cliente para ir buscar a sua chave publica em bytes para codifica a mensagem
                    ciphertext = encrypt_with_certificate(certificado, mensagem_codificada)
                    mensagem = mkpair(ciphertext, sigC)
                    ...
                    sys.stderr.write("MSG RELAY SERVICE: recipient client disconnected\n")
                    return None
            ...
            if dados == b'erro':
                ...
                for i, mensagens in enumerate(dadosFROMBSON["pedido"]):
                    ...
                    if mensagem["lida"] == 0 :
                        ...
                        plaintext = decrypt_with_private_key(message, self.private_key_rsa)
                        subject, content = unpair(plaintext)

                        #Valida mensagem recebida
                        if not valida_assinatura2(certSender, sigSender, plaintext):
                            sys.stderr.write("MSG RELAY SERVICE: verification error!\n")
                        else:
        ...
        if self.args[0] == 'getmsg':
            if dados == b'erro':
                ...
                if not valida_assinatura2(certSender, sigSender, plaintext):
                    ...
                    print("Leitura concluida:", content.decode())
    ...
    def process(self, msg=b""):
        ...
        if self.msg_cnt == 1:
            ...
            received_data, self.received_certS = unpair(msg)
            self.received_public_Key_Server, received_sigS = unpair(received_data)

            server_cert_reloaded = x509.load_pem_x509_certificate(self.received_certS)
            ...
            if not valida_assinatura(self.received_certS, received_sigS, self.received_public_Key_Server, self.serialized_public_key):
                return None
        ...
        #RECEBER E ENVIAR MENSAGENS AO SERVIDOR
        elif self.msg_cnt == 3:
            return self.handle_send_message(None, 0)
        
        elif self.msg_cnt == 4:
            resposta = decode(msg, self.derive_key)
            respostaFROMBSON = from_bson(resposta)
            received_data, recibo = unpair(respostaFROMBSON['resposta'])
            
            #valida da mensagem enviada
            if recibo != b"":
                validaRecibo(recibo, self.received_certS)
            
            tipo, dados = unpair(received_data)
            # Enviar a mensagem do send depois de ter recebido o certificado do destinatário
            if tipo.decode() == 'send':
                if dados == b'erro':
                    cert_decode = None
                else:
                    cert_decode = decode(dados, self.derive_key)
                return self.handle_send_message(cert_decode, 1)
            # Receber resposta do servidor ao askqueue, getmsg e help
            else:
                return self.handle_received_message(tipo, dados)
        #Receber resposta do servidor ao send
        else:
            resposta = decode(msg, self.derive_key)
            respostaFROMBSON = from_bson(resposta)
            received_data, recibo = unpair(respostaFROMBSON['resposta'])
